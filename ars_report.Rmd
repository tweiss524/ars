...
For $s_k(x)$ we approximate density function by calculating the area of under the piecewise $\exp(u_j(x))$ for each segment $(z_{j},z_{z+1})$, and get the unnormalized probability $q_j$.
...
The noramlized probability is derived by dividing each $q_j$ wit the summation of all unnormalized probability $A=\sum_{j=1}^nq_j$. The normalized probability, where the true probability we used to sample $x_*$of each segment $(z_j,z_{j+1})$ therefore equals to $$p_j = \frac{q_j}{A}a$$ 
...
We use the same method of sampling as Gilks and Wild derive. That is, we sample a $x^*$ from our `sample_sk()` function. first, we sample a point $w$ from a $\mathrm{U}(0, 1)$ distribution which represents the probability of $x$ equals to $x^*$.
...
\
We also wrote an R script called NAME OF R SCRIPT HERE to test that our auxiliary functions; namely LIST FUNCTIONS HERE. DESCRIBE PROCEDURE ONCE TESTS ARE WRITTEN.
